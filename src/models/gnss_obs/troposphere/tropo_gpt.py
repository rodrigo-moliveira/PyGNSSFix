...
class GPT3Tropo:
    ...
    def gpt3_5_fast_readGrid(self, filename):
        ...
        self.gpt3_grid[0] = p_grid
        self.gpt3_grid[1] = T_grid
        self.gpt3_grid[2] = Q_grid
        self.gpt3_grid[3] = dT_grid
        self.gpt3_grid[4] = u_grid
        self.gpt3_grid[5] = Hs_grid
        self.gpt3_grid[6] = ah_grid
        self.gpt3_grid[7] = aw_grid
        self.gpt3_grid[8] = la_grid
        self.gpt3_grid[9] = Tm_grid
        self.gpt3_grid[10] = Gn_h_grid
        self.gpt3_grid[11] = Ge_h_grid
    ...
    def gpt3_5_fast(self, mjd=None, lat=None, lon=None, h_ell=None, it=None):
        ...
        #
        #     Reference:
        #     D. Landskron, J. Böhm (2018), VMF3/troposphere: Refined Discrete and Empirical Troposphere Mapping Functions,
        ...
        p_grid = self.gpt3_grid[0]

        T_grid = self.gpt3_grid[1]

        Q_grid = self.gpt3_grid[2]

        dT_grid = self.gpt3_grid[3]

        u_grid = self.gpt3_grid[4]

        Hs_grid = self.gpt3_grid[5]

        ah_grid = self.gpt3_grid[6]

        aw_grid = self.gpt3_grid[7]

        la_grid = self.gpt3_grid[8]

        Tm_grid = self.gpt3_grid[9]

        Gn_h_grid = self.gpt3_grid[10]

        Ge_h_grid = self.gpt3_grid[11]

        Gn_w_grid = self.gpt3_grid[12]

        Ge_w_grid = self.gpt3_grid[13]
        ...
        for k in range(0, nstat):
            ...
            if bilinear == 0:
                ...
                T0 = T_grid.iloc[ix, 0] + T_grid.iloc[ix, 1] * cosfy + T_grid.iloc[ix, 2] * sinfy + T_grid.iloc[
                    ix, 3] * coshy + T_grid.iloc[ix, 4] * sinhy
                p0 = p_grid.iloc[ix, 0] + p_grid.iloc[ix, 1] * cosfy + p_grid.iloc[ix, 2] * sinfy + p_grid.iloc[
                    ix, 3] * coshy + p_grid.iloc[ix, 4] * sinhy
                # specific humidity
                Q = Q_grid.iloc[ix, 0] + Q_grid.iloc[ix, 1] * cosfy + Q_grid.iloc[ix, 2] * sinfy + Q_grid.iloc[
                    ix, 3] * coshy + Q_grid.iloc[ix, 4] * sinhy
                # lapse rate of the temperature
                dT[k] = dT_grid.iloc[ix, 0] + dT_grid.iloc[ix, 1] * cosfy + dT_grid.iloc[ix, 2] * sinfy + dT_grid.iloc[
                    ix, 3] * coshy + dT_grid.iloc[ix, 4] * sinhy
                ...
                ah[k] = ah_grid.iloc[ix, 0] + ah_grid.iloc[ix, 1] * cosfy + ah_grid.iloc[ix, 2] * sinfy + ah_grid.iloc[
                    ix, 3] * coshy + ah_grid.iloc[ix, 4] * sinhy
                aw[k] = aw_grid.iloc[ix, 0] + aw_grid.iloc[ix, 1] * cosfy + aw_grid.iloc[ix, 2] * sinfy + aw_grid.iloc[
                    ix, 3] * coshy + aw_grid.iloc[ix, 4] * sinhy
                # water vapour decrease factor la
                la[k] = la_grid.iloc[ix, 0] + la_grid.iloc[ix, 1] * cosfy + la_grid.iloc[ix, 2] * sinfy + la_grid.iloc[
                    ix, 3] * coshy + la_grid.iloc[ix, 4] * sinhy
                # mean temperature Tm
                Tm[k] = Tm_grid.iloc[ix, 0] + Tm_grid.iloc[ix, 1] * cosfy + Tm_grid.iloc[ix, 2] * sinfy + Tm_grid.iloc[
                    ix, 3] * coshy + Tm_grid.iloc[ix, 4] * sinhy
                # north and east gradients (total, hydrostatic and wet)
                Gn_h[k] = Gn_h_grid.iloc[ix, 0] + Gn_h_grid.iloc[ix, 1] * cosfy + Gn_h_grid.iloc[ix, 2] * sinfy + \
                          Gn_h_grid.iloc[ix, 3] * coshy + Gn_h_grid.iloc[ix, 4] * sinhy
                Ge_h[k] = Ge_h_grid.iloc[ix, 0] + Ge_h_grid.iloc[ix, 1] * cosfy + Ge_h_grid.iloc[ix, 2] * sinfy + \
                          Ge_h_grid.iloc[ix, 3] * coshy + Ge_h_grid.iloc[ix, 4] * sinhy
                Gn_w[k] = Gn_w_grid.iloc[ix, 0] + Gn_w_grid.iloc[ix, 1] * cosfy + Gn_w_grid.iloc[ix, 2] * sinfy + \
                          Gn_w_grid.iloc[ix, 3] * coshy + Gn_w_grid.iloc[ix, 4] * sinhy
                Ge_w[k] = Ge_w_grid.iloc[ix, 0] + Ge_w_grid.iloc[ix, 1] * cosfy + Ge_w_grid.iloc[ix, 2] * sinfy + \
                          Ge_w_grid.iloc[ix, 3] * coshy + Ge_w_grid.iloc[ix, 4] * sinhy
                ...
                hgt = h_ell[k] - undul
                # pressure, temperature at the height of the grid
                T0 = T_grid.iloc[indx, 0] + T_grid.iloc[indx, 1] * cosfy + T_grid.iloc[indx, 2] * sinfy + T_grid.iloc[
                    indx, 3] * coshy + T_grid.iloc[indx, 4] * sinhy
                p0 = p_grid.iloc[indx, 0] + p_grid.iloc[indx, 1] * cosfy + p_grid.iloc[indx, 2] * sinfy + p_grid.iloc[
                    indx, 3] * coshy + p_grid.iloc[indx, 4] * sinhy
                # humidity
                Ql = Q_grid.iloc[indx, 0] + Q_grid.iloc[indx, 1] * cosfy + Q_grid.iloc[indx, 2] * sinfy + Q_grid.iloc[
                    indx, 3] * coshy + Q_grid.iloc[indx, 4] * sinhy
                # reduction = stationheight - gridheight
                Hs1 = Hs_grid[indx]
                redh = hgt - Hs1
                # lapse rate of the temperature in degree / m
                dTl = dT_grid.iloc[indx, 0] + dT_grid.iloc[indx, 1] * cosfy + dT_grid.iloc[indx, 2] * sinfy + \
                      dT_grid.iloc[
                          indx, 3] * coshy + dT_grid.iloc[indx, 4] * sinhy
                ...
                # hydrostatic and wet coefficients ah and aw
                ahl = ah_grid.iloc[indx, 0] + ah_grid.iloc[indx, 1] * cosfy + ah_grid.iloc[indx, 2] * sinfy + \
                      ah_grid.iloc[
                          indx, 3] * coshy + ah_grid.iloc[indx, 4] * sinhy
                awl = aw_grid.iloc[indx, 0] + aw_grid.iloc[indx, 1] * cosfy + aw_grid.iloc[indx, 2] * sinfy + \
                      aw_grid.iloc[
                          indx, 3] * coshy + aw_grid.iloc[indx, 4] * sinhy
                # water vapour decrease factor la
                lal = la_grid.iloc[indx, 0] + la_grid.iloc[indx, 1] * cosfy + la_grid.iloc[indx, 2] * sinfy + \
                      la_grid.iloc[
                          indx, 3] * coshy + la_grid.iloc[indx, 4] * sinhy
                # mean temperature of the water vapor Tm
                Tml = Tm_grid.iloc[indx, 0] + Tm_grid.iloc[indx, 1] * cosfy + Tm_grid.iloc[indx, 2] * sinfy + \
                      Tm_grid.iloc[
                          indx, 3] * coshy + Tm_grid.iloc[indx, 4] * sinhy
                # north and east gradients (total, hydrostatic and wet)
                Gn_hl = Gn_h_grid.iloc[indx, 0] + Gn_h_grid.iloc[indx, 1] * cosfy + Gn_h_grid.iloc[indx, 2] * sinfy + \
                        Gn_h_grid.iloc[indx, 3] * coshy + Gn_h_grid.iloc[indx, 4] * sinhy
                Ge_hl = Ge_h_grid.iloc[indx, 0] + Ge_h_grid.iloc[indx, 1] * cosfy + Ge_h_grid.iloc[indx, 2] * sinfy + \
                        Ge_h_grid.iloc[indx, 3] * coshy + Ge_h_grid.iloc[indx, 4] * sinhy
                Gn_wl = Gn_w_grid.iloc[indx, 0] + Gn_w_grid.iloc[indx, 1] * cosfy + Gn_w_grid.iloc[indx, 2] * sinfy + \
                        Gn_w_grid.iloc[indx, 3] * coshy + Gn_w_grid.iloc[indx, 4] * sinhy
                Ge_wl = Ge_w_grid.iloc[indx, 0] + Ge_w_grid.iloc[indx, 1] * cosfy + Ge_w_grid.iloc[indx, 2] * sinfy + \
                        Ge_w_grid.iloc[indx, 3] * coshy + Ge_w_grid.iloc[indx, 4] * sinhy
                # water vapor pressure in hPa

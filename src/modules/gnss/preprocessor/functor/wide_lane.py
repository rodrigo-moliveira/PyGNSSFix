""" Module with the implementation of the Wide-Lane Functor """
...
class WLFunctor(Functor):
    """
    The `WLFunctor` computes wide-lane observations.

    The pseudorange and carrier phase wide-lane combinations are computed with the following equations:

        PR_WL = f1 * PR1 / (f1 - f2) - f2 * PR2 / (f1 - f2)
        CP_WL = f1 * CP1 / (f1 - f2) - f2 * CP2 / (f1 - f2)
    ...
    def __init__(self, constellation: str, base_freq: DataType, second_freq: DataType):
        ...
        self.cp2 = None
        self.cp_wl = None
        self._compute_wl_std(constellation)

    def _compute_wl_std(self, constellation):
        ...
        if self.pr1 is not None and self.pr2 is not None:
            ...
            # Eq. (20.29) from [1]
            pr_std_wl = sqrt((f1**2 * pr_std_1**2 + f2**2 * pr_std_2**2) / (f1 - f2)**2)
            cp_std_wl = sqrt((f1**2 * cp_std_1**2 + f2**2 * cp_std_2**2) / (f1 - f2)**2)
            self.pr_wl = DataType.get_wl_datatype(self.pr1, self.pr2, constellation)
            self.cp_wl = DataType.get_wl_datatype(self.cp1, self.cp2, constellation)
            config_dict.update_obs_std(constellation, self.pr_wl, pr_std_wl)
            config_dict.update_obs_std(constellation, self.cp_wl, cp_std_wl)

            log = get_logger(MODEL_LOG)
            log.info(f"Computing wide-lane observation std for constellation {constellation}. {self.pr1}: {pr_std_1}"
                     f" - {self.pr2}: {pr_std_2} -> {self.pr_wl}: {pr_std_wl} and {self.cp1}: {cp_std_1} - "
                        f"{self.cp2}: {cp_std_2} -> {self.cp_wl}: {cp_std_wl}")
        else:
    ...
    @staticmethod
    def _compute_wl(obs1: Observation, obs2: Observation) -> float:
        """ Computes the wide-lane combination (value) """
        f1 = obs1.datatype.freq.freq_value
        f2 = obs2.datatype.freq.freq_value
        # Eq. (20.24) of [1]
        wl_obs = (f1 * obs1.value - f2 * obs2.value) / (f1 - f2)
        return wl_obs
    ...
    def __call__(self, obs_data_in, epoch, sat):
        ...
        if C1 is not None and C2 is not None:
            wl_code = self._compute_wl(C1, C2)
            v_obs_out.append(Observation(self.pr_wl, wl_code))

        # get wide-lane carrier
        if L1 is not None and L2 is not None:
            wl_carrier = self._compute_wl(L1, L2)
            v_obs_out.append(Observation(self.cp_wl, wl_carrier))
